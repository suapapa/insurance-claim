...
1. **서류 구분** — 비전 LLM이 환자 / 날짜 / 진단명(또는 사고) 기준으로 그룹을 묶고,
   `data/claims/YYYY-MM-DD_환자_진단명/` 디렉터리에 업로드 원본을 가리키는 상대 심볼릭 링크를 만듭니다.
...
원본 이미지와 `summary.yaml`이 담긴 ZIP 파일로 다운로드할 수 있습니다. 목록에서는 업로드 때
생성한 최대 320px JPEG 썸네일을 사용하며, 작업 카드의 **삭제** 버튼으로 원본·썸네일·파생
청구 폴더를 한 번에 삭제할 수 있습니다.
...
data/
  ...
  claims/<청구>/     업로드 원본의 상대 심볼릭 링크 + summary.yaml
```
...
- 원본은 `data/uploads/`에 한 번만 저장되고, 청구 폴더에서는 **상대 심볼릭 링크**로 참조합니다.
- 청구 ZIP을 만들 때는 심볼릭 링크가 아니라 링크가 가리키는 원본 파일을 포함합니다.
